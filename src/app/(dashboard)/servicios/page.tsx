              ...
              {servicios.map((s) => (
                <tr key={s.id} className="border-b border-slate-100 dark:border-slate-800 last:border-0">
                  <td className="px-5 py-3">
                    ...
                    {s.descripcion && (
                      <div className="text-xs text-slate-500 dark:text-slate-400">{s.descripcion}</div>
                    )}
                  </td>
                  <td className="px-5 py-3 text-slate-600 dark:text-slate-300">
                    <div>Base: ${formatMoney(s.precioBase)}</div>
                    {s.unidad ? (
                      <div className="text-xs text-slate-500 dark:text-slate-400">
                        ${formatMoney(s.precioPorUnidad)} / {s.unidad}
                        {s.incluidos > 0 && ` · ${s.incluidos} incluidos`}
                      </div>
                    ) : (
                      <div className="text-xs text-slate-400 dark:text-slate-500">costo fijo</div>
                    )}
                  </td>
                  <td className="px-5 py-3">
                    <span
                  ...
                  <td className="px-5 py-3 text-right">
                    ...
                    <button
                      onClick={() => eliminar(s)}
                      className="ml-4 font-medium text-red-600 dark:text-red-400 hover:text-red-500"
                    >
                      Eliminar
                    </button>
                  </td>
          ...
          <Card className="w-full max-w-md">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <label className="block">
                  <span className="text-sm text-slate-600 dark:text-slate-300">Precio por unidad</span>
                  <MoneyInput
              ...
              <div className="grid grid-cols-2 gap-3">
                <label className="block">
                  <span className="text-sm text-slate-600 dark:text-slate-300">
                    Unidad{" "}
                    <span className="text-slate-400 dark:text-slate-500">(vacío = costo fijo)</span>
                  </span>
                  <input
                    value={form.unidad}
