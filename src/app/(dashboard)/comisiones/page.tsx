...
export default function ComisionesPage() {
  ...
  const cargar = useCallback(async () => {
    ...
    try {
      const [cs, ps] = await Promise.all([
        ventasApi.comisiones({ estado: fEstado || undefined }),
  ...
  useEffect(() => { if (esAdmin) ventasApi.comerciales().then(setComerciales); }, [esAdmin]);
  ...
  return (
    <div>
      <PageHeader title="Comisiones" subtitle="Comisiones generadas por las ventas de planes." />
            ...
            <tbody>
              {items.map((c) => (
                <tr key={c.id} className="border-b border-slate-100 dark:border-slate-800 last:border-0">
                  ...
                  {esAdmin && <td className="px-5 py-3 text-slate-600 dark:text-slate-300">{comercialDe(c.comercialId)}</td>}
                    ...
                    <td className="px-5 py-3 text-right whitespace-nowrap">
                      {c.estado === "PENDIENTE" && (
                        <span className="flex justify-end gap-3">
                          <button onClick={() => pagar(c)} className="font-medium text-emerald-600 dark:text-emerald-400 hover:underline">Marcar pagada</button>
                          <button onClick={() => anular(c)} className="font-medium text-red-600 dark:text-red-400 hover:underline">Anular</button>
                        </span>
                      )}
                    </td>
                  )}
